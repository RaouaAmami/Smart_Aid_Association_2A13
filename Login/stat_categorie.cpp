...
void stat_categorie::choix_pie()
{
    ...
    QSqlQuery q1,q2;
    qreal tot=0,c;
    q1.prepare("SELECT DISTINCT TYPE FROM EVENEMENTS");    //Pour éviter des redondances dans les résultats il faut simplement ajouter DISTINCT après le mot SELECT
    q1.exec();
    while (q1.next()){
    ...
    for (auto i = liste_cat.begin(); i != liste_cat.end(); ++i) {
         q2.prepare("SELECT * FROM EVENEMENTS where TYPE = :m");
         q2.bindValue(":m", *i);
         q2.exec();
         c=0;
         while (q2.next()){c++;}
